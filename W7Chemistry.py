...
def main():
    print_slow("Welcome! This is a program that computes the molar mass ")
    print_slow("of a substance and the number of moles of a sample of that substance")
    print()
    question = input("If you wish to continue type in Y/N : ")
    print()
    if question.lower() == "y":
        # Get a chemical formula for a molecule from the user.
        formula = input("Please Enter the molecular formula of the sample: ")
        ...
        list_values = parse_formula(formula, table)
        # Call the compute_molar_mass function to compute the
        # molar mass of the molecule from the compound list.
        compute_molar_mass(list_values, table)
        # Compute the number of moles of the sample.
        print(f"{compute_molar_mass(list_values, table):.5f} grams/mole")
        # Print the molar mass.
        number_of_moles = float(float(mass_in_grams) / float(compute_molar_mass(list_values, table)))
        print(f"The number of Moles: {number_of_moles:.5f}")
        # Print the number of moles.
    else:
        print("Thanks for your time! Try using it Next Time!!")
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for i in range(len(symbol_quantity_list)):
        symbol = symbol_quantity_list[i][0]
        quantity = symbol_quantity_list[i][1]
        key_symbol = periodic_table[symbol]
        atomic_mass = key_symbol[ATOMIC_MASS_INDEX]
